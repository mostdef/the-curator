...
function getViewList(view)       { return view === 'collection' ? tvSeries : ({ watchlist: loadWatchlist, maybe: loadMaybe, meh: loadMeh, banned: loadBanned }[view])(); }
function saveViewList(view, list){ if (view === 'collection') { tvSeries.splice(0, tvSeries.length, ...list); saveSeries(); } else ({ watchlist: saveWatchlist, maybe: saveMaybe, meh: saveMeh, banned: saveBanned }[view])(list); }

function moveBetweenViews(title, fromView, toView) {
  const src = getViewList(fromView);
  const idx  = src.findIndex(m => m.title === title);
  if (idx === -1) return;
  const [series] = src.splice(idx, 1);
  saveViewList(fromView, src);
  series.addedAt = Date.now();
...
function renderStandardsSection() {
  ...
  title.textContent = 'Reference Series';
  ...
  for (let i = 0; i < MAX_STANDARDS; i++) {
    ...
    if (series) {
      ...
      img.title = `${series.title} (${series.year})`;
      img.draggable = false;
      ...
      removeBtn.addEventListener('click', () => {
        const updated = loadStandards().filter(m => m.title !== series.title);
        saveStandards(updated);
        render(sortedList(tvSeries, 'collection'));
        applyGrain();
      });
...
const PERSONA_CACHE_KEY = 'braintrust_series_persona_cache';
...
function renderPersonaCard(wrap, data) {
  ...
  label.textContent = 'Your TV Persona';
...
function toggleStandard(series) {
  const stds = loadStandards();
  const idx = stds.findIndex(m => m.title === series.title);
  if (idx !== -1) {
    stds.splice(idx, 1);
  } else {
    if (stds.length >= MAX_STANDARDS) return false;
    stds.push({ title: series.title, year: series.year, creator: series.creator, poster: series.poster });
  }
  saveStandards(stds);
  return true;
}
...
function render(list) {
  ...
  list.forEach(series => {
    const card = document.createElement('div');
    const isStandard = standards.some(m => m.title === series.title);
    ...
    const textures = getCachedTextures(series.title);
    ...
    starBtn.title = isStandard ? 'Remove from Reference Series' : 'Add to Reference Series';
    starBtn.innerHTML = '★';
    starBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const ok = toggleStandard(series);
      if (ok !== false) { render(sortedList(tvSeries, 'collection')); applyGrain(); }
    });
    ...
    meta.textContent = `${series.creator}, ${series.year}`;
...
async function fetchRecommendation() {
  ...
  try {
    const excluded = [
      ...tvSeries.map(m => m.title),
      ...loadBanned().map(m => m.title),
      ...loadWatchlist().map(m => m.title),
      ...sessionExcluded,
    ];

    const res = await fetch('/api/recommend-series', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ series: tvSeries, excluded, standards: loadStandards() }),
    });
...
function renderRecommendation() {
  ...
  meta.textContent = `${rec.creator}, ${rec.year}`;
  ...
  watchBtn.addEventListener('click', () => {
    ...
    if (!list.some(m => m.title === rec.title)) {
      list.unshift({ title: rec.title, year: rec.year, creator: rec.creator, poster: rec.poster, addedAt: Date.now() });
      saveWatchlist(list);
    }
  ...
  addBtn.textContent = 'Already Watched';
  addBtn.addEventListener('click', () => {
    tvSeries.unshift({ title: rec.title, year: rec.year, creator: rec.creator, poster: rec.poster, addedAt: Date.now() });
    saveSeries();
    render(tvSeries);
    applyGrain();
    fetchRecommendation();
...
// Persistence
const STORAGE_KEY    = 'braintrust_series';
const BANNED_KEY     = 'braintrust_series_banned';
const WATCHLIST_KEY  = 'braintrust_series_watchlist';
const MAYBE_KEY      = 'braintrust_series_maybe';
const MEH_KEY        = 'braintrust_series_meh';
const SNAPSHOTS_KEY  = 'braintrust_series_snapshots';
const STANDARDS_KEY  = 'braintrust_series_standards';
...
function saveSnapshot(label = '') {
  const snap = {
    ...
    series:    JSON.parse(localStorage.getItem(STORAGE_KEY)   || '[]'),
...
function restoreSnapshot(snap) {
  localStorage.setItem(STORAGE_KEY,   JSON.stringify(snap.series));
  ...
  tvSeries.splice(0, tvSeries.length, ...snap.series);
...
const SORT_KEY = 'braintrust_series_sort';
  ...
  sortableInstance = Sortable.create(el, {
    ...
    onEnd: () => {
      ...
      if (pendingStandardsSlot && draggedCard) {
        ...
        const series  = tvSeries.find(m => m.title === title);
        if (series) {
          const stds = loadStandards();
...
function setGridView(view) {
  ...
  if (dirtyViews.has(view)) {
    if (view === 'collection') { render(sortedList(tvSeries, 'collection')); applyGrain(); }
...
function getTabCount(key) {
  if (key === 'collection') return tvSeries.length;
...
function renderWatchlistGrid() {
  ...
  list.forEach(series => {
    ...
    removeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      saveSnapshot(`Before removing "${series.title}" from To Watch`);
      const prev = loadWatchlist();
      const updated = prev.filter(m => m.title !== series.title);
      saveWatchlist(updated);
      renderWatchlistGrid();
      renderGridNav();
      showUndo(`Removed "${series.title}" from To Watch`, () => {
        saveWatchlist(prev);
        renderWatchlistGrid();
        renderGridNav();
      });
    ...
    posterWrap.appendChild(img);
    addTexturesToPoster(posterWrap, series.title);
...
function renderBannedGrid() {
  ...
  banned.forEach(series => {
    ...
    removeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      saveSnapshot(`Before removing "${series.title}" from Don't Recommend`);
      const prev = loadBanned();
      const list = prev.filter(m => m.title !== series.title);
      saveBanned(list);
      renderBannedGrid();
      renderGridNav();
      showUndo(`Removed "${series.title}" from Don't Recommend`, () => {
        saveBanned(prev);
        renderBannedGrid();
        renderGridNav();
      });
...
function renderMaybeGrid() {
  ...
  list.forEach(series => {
    ...
    removeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      saveSnapshot(`Before removing "${series.title}" from Wildcard`);
      const prev = loadMaybe();
      const updated = prev.filter(m => m.title !== series.title);
      saveMaybe(updated);
      renderMaybeGrid();
      renderGridNav();
      showUndo(`Removed "${series.title}" from Wildcard`, () => {
        saveMaybe(prev);
        renderMaybeGrid();
        renderGridNav();
      });
...
function renderMehGrid() {
  ...
  list.forEach(series => {
    ...
    removeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      saveSnapshot(`Before removing "${series.title}" from Meh`);
      const prev = loadMeh();
      const updated = prev.filter(m => m.title !== series.title);
      saveMeh(updated);
      renderMehGrid();
      renderGridNav();
      showUndo(`Removed "${series.title}" from Meh`, () => {
        saveMeh(prev);
        renderMehGrid();
        renderGridNav();
      });
...
function saveSeries() {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(tvSeries));
...
function loadSeries() {
  ...
  try {
    ...
    tvSeries.splice(0, tvSeries.length, ...parsed);
  } catch (e) {}
}
...
function syncOrderFromDOM() {
  ...
  cards.forEach(card => {
    ...
    const series = tvSeries.find(m => m.title === title);
    if (series) newOrder.push(series);
  });
  tvSeries.splice(0, tvSeries.length, ...newOrder);
  saveSeries();
}
